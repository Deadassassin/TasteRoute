...
data class PlaceFacts(
    ...
    /**
     * Real dish names from the venue's own schema.org MenuItem markup (server >= 1.3.0, capability
     * `menus`). Empty for most venues and that is correct — a menu published as a PDF or through an
     * ordering widget has no structured data in it, and a guessed dish shown as fact is a lie about
     * a restaurant. [DishPicks] handles the empty case by suggesting one and saying that it is a
     * suggestion.
     */
...
data class RestaurantResult(
    ...
    /**
     * The venue's OWN site, or null.
     *
     * [webUrl] falls back to an openstreetmap.org permalink for the very many places that carry no
     * website tag. That is a fine "look this place up" link and the wrong answer to "where does
     * this restaurant publish its menu" — and it was being handed to the menu harvester as the
     * restaurant's website. The harvester dutifully fetched OpenStreetMap, found no menu markup on
     * it (there is none), and cached that miss against the restaurant for a fortnight. Which is
     * why the menu screen was empty for nearly everything, whatever the kitchen actually publishes.
     */
    val venueSite: String? get() = webUrl
        ?.takeIf { it.isNotBlank() }
        ?.takeUnless { it.contains("openstreetmap.org", ignoreCase = true) }
}
...
object Entitlements {
    ...
    /**
     * Searching a named city is not the same job as searching around your feet, and the nearby cap
     * is the wrong number for it — three kilometres of Las Vegas is one boulevard, which is why
     * asking about a city you are not standing in used to come back with almost nothing. It is
     * deliberately NOT a tier lever: the search is centred on the city itself, so a wide radius
     * costs the same single call, and gating it would only make the free tier wrong rather than
     * smaller.
     */
