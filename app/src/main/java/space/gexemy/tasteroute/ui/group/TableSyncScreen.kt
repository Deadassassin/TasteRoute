package space.gexemy.tasteroute.ui.group
...
fun TableSyncScreen(onBack: () -> Unit, onOpenPlace: () -> Unit, onSignIn: () -> Unit) {
    ...
    Box(Modifier.fillMaxSize()) {
        Column(
            ...
            Text("Table Sync", style = MaterialTheme.typography.headlineSmall)
            ...
            if (!Session.signedIn) {
                ...
                if (picks.isNotEmpty()) {
                    Text("Works for the table", style = MaterialTheme.typography.titleMedium)
                    picks.forEach { place ->
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            RestaurantCard(
                                ...
                                onToggleFavorite = { AppState.toggleFavorite(place) },
                                onClick = {
...
private fun MergeSummary(merged: GexemyClient.MergedTaste) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Text("What the table agrees on", style = MaterialTheme.typography.titleMedium)
