...
object AppState {
    /** Real device fix. Null until the user grants location — nothing is recommended before then. */
    ...
    fun rememberWarm(results: List<RestaurantResult>, request: RecommendationRequest) {
        ...
        val keep = results.take(WARM_LIMIT)
        warmResults = keep
    ...
    /** Called once from Application.onCreate, before any composition reads this state. */
    fun restore() {
